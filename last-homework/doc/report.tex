...
\begin{figure}[h]
  \subfigure[牛顿法的流程图]{
    \begin{tikzpicture}[scale=0.5,node distance=10pt]
    \tikzstyle{every node}=[font=\small,scale=0.5]
      \node[draw, rounded corners] (start) {开始,设置最大迭代次数$N$};
      \node[draw, below=of start] (step 1) {输入一个函数,并猜想它的根$x_0$};
      \node[draw, below=of step 1] (step 2) {$x_{n+1}=x_n-f(x_n)/f(x_n)'$};
      \node[draw, diamond, aspect=2, below=of step 2] (choice) {是否$n>N$,或者精度足够?};
      \node[draw, right=30pt of choice] (step x) {$x_n=x_{n+1};n=n+1$};
      \node[draw, rounded corners, below=20pt of choice] (end) {结束,$x_n$为所求结果};
  ...
  \subfigure[二分法]{
    ...
    \tikzstyle{every node}=[font=\small,scale=0.5]
      \node[draw, rounded corners] (start) {开始,设置最大迭代次数$N$};
      \node[draw, below=of start] (step 1) {输入一个函数,并猜想它的根$\in I_0$,$I_0$为包含根且两端函数值异号的区间};
      \node[draw, below=of step 1] (step 2) {$I_{n+1}=1/2*I_n$，满足两端函数值异号};
      \node[draw, diamond, aspect=2, below=of step 2]     (choice)  {是否$n>N$,或者区间足够小?};
      \node[draw, right=30pt of choice]                   (step x)  {$I_n=I_{n+1};n=n+1$};
      \node[draw, rounded corners, below=20pt of choice]  (end)     {结束,$I_n$的中点$x_n$为所求结果};
